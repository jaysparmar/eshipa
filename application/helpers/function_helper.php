...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL, $partner_id = NULL, $sort_by = "sd.user_id", $buy_stock = NULL)
{
    ...
    $t->db->select($discount_filter_data . ' (select count(id)  from products where products.category_id=c.id ) as total,count(p.id) as sales, p.stock_type,p.calories,p.status ,p.is_prices_inclusive_tax,p.tax as tax_id, p.type ,GROUP_CONCAT(DISTINCT(pa.attribute_value_ids)) as attr_value_ids, p.partner_id,p.id,p.stock,p.name,p.category_id,p.short_description,p.slug,p.total_allowed_quantity,p.minimum_order_quantity,p.cod_allowed,p.row_order,p.rating,p.no_of_ratings,p.image,p.is_cancelable,p.cancelable_till,p.indicator, p.highlights,p.availability,c.name as category_name,c.slug as category_slug,p.available_time,p.start_time,p.end_time,tax.percentage as tax_percentage ')
        ...
        ->join(" partner_data sd", "p.partner_id=sd.user_id ", $t->ion_auth->is_admin() || $buy_stock ? 'LEFT' : '')
        ->join(" users u", "p.partner_id=u.id", $t->ion_auth->is_admin() || $buy_stock ? 'LEFT' : '')
    ...
    if (isset($filter) && !empty($filter['partner_slug']) && $filter['partner_slug'] != "") {
        $where['sd.slug'] = $filter['partner_slug'];
    }
    //category slug
    ...
    if (isset($filter) && $filter['vegetarian'] != "") {
        if ($filter['vegetarian'] == "3") {
            ...
            $t->db->where('p.indicator ', $filter['vegetarian']);
            $t->db->where_in('sd.type ', [$filter['vegetarian'], 3]);
        }
    }
...
function fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $rider_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $download_thermal_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null, $partner_id = null, $is_pending = false, $rider_city_id = NULL)
{
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        if ((isset($rider_id) && $rider_id != null) || (isset($partner_id) && $partner_id != null)) {
            $order_details[$i]['total'] = strval($item_subtotal + intval($total_tax_amount));
...
function get_user_balance($user_id, $field)
{
    $ci = &get_instance();
    $ci->db->where('id', $user_id);
    if ($field === 'balance') {
        $ci->db->select('balance');
    } elseif ($field === 'epoints') {
        $ci->db->select('epoints');
    }
    $res = $ci->db->get('users')->result_array();
    if (!empty($res)) {
        return !empty($res[0][$field]) ? $res[0][$field] : "0";
    } else {
        return "0";
    }
}
...
function fetch_partners($filter = [], $user_id = null, $limit = NULL, $offset = '', $sort = 'u.id', $order = 'DESC', $search = NULL)
{
    ...
    $search_res = $t->db->select($where_near_by_for_select . ' `u`.username as owner_name,,(SELECT MIN(if(pv.special_price != 0, pv.special_price, pv.price)) FROM `partner_data` pd JOIN products p on p.partner_id = pd.user_id JOIN product_variants pv ON pv.product_id=p.id WHERE pd.user_id=sd.user_id ) as price_for_one,u.id as partner_id,u.email,u.mobile,u.balance,sd.address as partner_address,u.city as city_id,c.name as city_name,c.time_to_travel,u.fcm_id,u.latitude,u.longitude,`sd`.* ')
        ->join('users_groups ug', ' ug.user_id = u.id ')
        ->join('partner_data sd', ' sd.user_id = u.id ')
        ->join('products p', ' p.partner_id = u.id ', "left")
        ->join('cities c', 'c.id = u.city', "left")
    ...
    foreach ($restro_search_res as $row) {
        ...
        $permmissions['delivery_orders'] = (isset($permit['delivery_orders']) && !empty($permit['delivery_orders'])) ? $permit['delivery_orders'] : "1";
